...
async function signUp() {
    ...
    try {
        ...
        console.log(user);
        alert(document.getElementById("username").value + " Signed up successfully. Contact Admin for confirmation.");
    } catch (error) {
        alert(error);
...
function loadAttendanceTime(row) {
    ...
    $("#attendanceUsr").text("email: " + row.name);
    //alert(" key -> " + row.id);
...
photoUpld.addEventListener('click', function() {
    $("#uploadAphoto").prop('disabled', true);
...
async function imageIsLoaded(file) {
    ...
    postData(ENDPOINT_URL + "/photos?type=uploadAndIndexImage&collectionId=blue-star-dorm-collection", body).then((res) => {
        ...
        if (res) {
            ...
            alert("User added successfully. Check details in ID extraction in 10 seconds.")
            clearImg();
        }
    });
}
...
function clearImg() {
    ...
    $("#newmail").val("");
    $("#image").prop("src","");
    $("#uploadAphoto").prop('disabled', false);
}
...
function genIDData(content){
    ...
    if(content === undefined){
        alert("Select a user to view ID details.")
    }
